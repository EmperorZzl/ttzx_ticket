package com.ttzx.ticket.view;

import com.ttzx.ticket.util.StrUtils;
import com.ttzx.ticket.util.ViewUtils;
...
public class TitleBar extends LinearLayout {
	...
	private void inintTitleBar(Context context) {
		...
		layoutParamsWW.gravity = Gravity.CENTER_VERTICAL;// 垂直居中
		...
		titleTextBtn.setPadding(5, 0, 5, 0);// 左上右下
		...
		titleSmallTextBtn.setPadding(6, 0, 5, 0);// 左上右下
	...
	public void setTitleBarGravity(int gravity1, int gravity2) {
		ViewUtils.measureView(this.rightLayout);
		ViewUtils.measureView(this.logoView);
		...
		Log.d("00000", "测量布局的宽度：" + leftWidth + "," + rightWidth);
		...
		if ((gravity1 == Gravity.CENTER_HORIZONTAL || gravity1 == Gravity.CENTER)) {
			...
			this.titleTextLayout.setGravity(Gravity.LEFT);
			this.rightLayout.setHorizontalGravity(Gravity.LEFT);
		} else if (gravity1 == Gravity.CENTER && gravity2 == Gravity.RIGHT) {
	...
	public void addRightView(View rightView,
		...
		if (layoutParams != null) {
			...
			rightLayout.addView(rightView, layoutParams);
		} else {
			rightLayout.setVisibility(View.VISIBLE);
			rightLayout.addView(rightView, layoutParamsWW);// 就是继承了父view的宽高
		}
